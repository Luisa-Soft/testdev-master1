...
companies = example1(companies);

cleanConsole(3, companies);
console.log('---- EXAMPLE 3 --- ', example3(companies));

//Compara si el nombre enviado tiene la primera en mayuscula
//Si alguno no cumple devuelve false
//Si todos cumplen devuelve true
function example3(comp) {
    for (i = 0; i < comp.length; i++) {
        if (comp[i].indexOf(comp[i].charAt(0).toUpperCase() + comp[i].slice(1)) != comp) {
            return false;
        } else {
            return true;
        }
